...
def generate_sen(config, model, model_sc, step, mode):
    ...
    for item in items:
        ...
        with open(item['out_txt'], 'w') as f:
            for i, (obj, obj_num, feat) in tqdm(enumerate(val_loader)):
                ...
                feat_vec = res_vec
                mu_pri, sigma2_pri = model.q_encoder.generate_prior(feat_vec)
                ...
                while True:
                    ...
                    latent_vec *= torch.sqrt(torch.exp(sigma2_pri))  # scaled
                    latent_vec += mu_pri  # shifted

                    pro = softmax(model_sc(latent_vec)[0])
                    num_sample += 1
                    if step > 40000:  # 40000步之前不强制要求制定风格，以免速度过慢
                        if pro[item['dim']] < 0.9 and num_sample < 2000:
                            continue
                    ...
                    sentence_id = model.generate_onesample(feat_vec, style_label, latent_vec)
...
def generate_sen_reval(config, model, model_sc, step, mode):
    """使用当前模型生成romantic和funny的句子并保存成json和txt格式以便后续计算B、M、C以及ppl和cls"""
    print("Generating sentence...")
        ...
        with open(item['out_txt'], 'w') as f:
            for i, (obj, obj_num, feat) in tqdm(enumerate(val_loader)):
                ...
                while True:
                    ...
                    pro = softmax(model_sc(latent_vec)[0])
                    num_sample += 1
                    if pro[item['dim']] < 0.9 and num_sample < 100:
                        continue
                    if pro[item['dim']] < 0.8 and num_sample < 200:
                        continue
                    if pro[item['dim']] < 0.7 and num_sample < 300:
                        continue
                    if pro[item['dim']] < 0.6 and num_sample < 400:
                        continue
                    if pro[item['dim']] < 0.5 and num_sample < 500:
                        continue
                    ...
                    # sentence_id = model.generate_onesample(feat_vec, style_label, latent_vec)
                    sentence_id, sentences = model.generate_beamsearch(feat_vec, style_label, latent_vec)
                    sentence_id = torch.Tensor(sentence_id)

                    # 检验reconstruction后的句子是否真正符合风格
                    length = 0
                    ...
                    if float(pred_pro[1]) < 0.9 and num_try <= 10:
                        continue
...
def generate_sen_test(config, model, model_sc, step, mode):
    """使用当前模型生成romantic和funny的句子并保存成json和txt格式以便后续计算B、M、C以及ppl和cls"""
    print("Generating sentence...")
        ...
        with open(item['out_txt'], 'w') as f:
            for i, (obj, obj_num, feat) in tqdm(enumerate(val_loader)):
                ...
                while True:
                    ...
                    # sentence_id = model.generate_onesample(feat_vec, style_label, latent_vec)
                    sentence_id, sentences_beam = model.generate_beamsearch(feat_vec, style_label, latent_vec)
                    sentence_id = torch.Tensor(sentence_id)
                    ...
                    if len(sentence_5) == 5 or num_try > 10:
                        break
                ...
                while len(sentence_5) < 5:
                    ...
                    if not tensor_found(sentence_5, sentence_id) :
                        sentence_5.append(sentence_id)
                    elif num_found > 100:
                        sentence_5.append(sentence_id)
...
def tensor_found(sentence_5, sentence_id):
    found = False
    for item in sentence_5:
        if item.equal(sentence_id):
            found = True
            break
    return found
